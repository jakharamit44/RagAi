...
- **UI Identifiers:** `<input id="quick-url-input">` and `<button onclick="quickIngestUrl()">Quick Ingest</button>`
- **Purpose & Use Case:** If an administrator receives an urgent university notice URL (e.g., an emergency holiday notice or updated exam date sheet), they do not need to run a full crawler pass. Entering the URL and clicking **Quick Ingest** immediately:
  1. Validates the URL against the SSRF firewall.
  2. Downloads the HTML page or PDF file.
  3. Converts HTML to structured Markdown or performs OCR on the PDF.
  4. Embeds the content into Qdrant.
  5. Updates the Cognitive Brain cortex with a new `Web Notice` node.
  6. Purges the temporary download file.

#### 3.11.6 Crawler Job Name Input (`crawler-job-name`)
- **UI Identifier:** `<input id="crawler-job-name" type="text">`
- **Purpose & Use Case:** Identifier for the crawling task (e.g., `MDU_Official_Portal_Sync`).

#### 3.11.7 Target Base URL Input (`crawler-base-url`)
- **UI Identifier:** `<input id="crawler-base-url" type="url" placeholder="https://mdu.ac.in">`
- **Purpose & Use Case:** Root domain URL used to resolve relative links.

#### 3.11.8 Seed URLs Textarea (`crawler-seed-urls`)
- **UI Identifier:** `<textarea id="crawler-seed-urls">`
...
#### 3.11.9 Allowed Domains Input (`crawler-allowed-domains`)
- **UI Identifier:** `<input id="crawler-allowed-domains" type="text">`
- **Purpose & Use Case:** Comma-separated list of fully qualified domains the crawler is permitted to visit. Prevents the crawler from wandering onto external web servers.

#### 3.11.10 Maximum Crawl Depth Number Box (`crawler-max-depth`)
- **UI Identifier:** `<input id="crawler-max-depth" type="number" value="2">`
...
#### 3.11.11 Maximum Pages Ceiling Number Box (`crawler-max-pages`)
- **UI Identifier:** `<input id="crawler-max-pages" type="number" value="100">`
- **Purpose & Use Case:** Caps the total number of pages crawled per run to prevent runaway crawls.

#### 3.11.12 Automated Crawl Interval Number Box (`crawler-interval`)
- **UI Identifier:** `<input id="crawler-interval" type="number" value="30">`
- **Purpose & Use Case:** Frequency in minutes between automated crawler runs when operating in daemon mode.

#### 3.11.13 "Auto-Ingest Discovered Documents" Toggle Checkbox (`crawler-auto-ingest`)
- **UI Identifier:** `<input id="crawler-auto-ingest" type="checkbox" checked>`
...
2. `data/qdrant_storage/` (or the persistent Qdrant volume holding all 1024-dimensional vector points).

#### Q4: How does the Cognitive AI Brain differ from standard vector search?
